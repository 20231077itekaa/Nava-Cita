...
<div class="p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="bg-blue-50 p-6 rounded-xl shadow-md border border-blue-300">
            <div class="flex items-center">
                {{-- Pastikan Font Awesome (fas) di-load di layout admin --}}
                <i class="fas fa-images text-blue-500 text-2xl mr-4"></i>
                <div>
                    <p class="text-sm font-medium text-gray-500">Total Foto</p>
                    <p class="text-2xl font-semibold text-gray-900">{{ $totalFoto }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-green-50 p-6 rounded-xl shadow-md border border-green-300">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $totalMasukan }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        @foreach($fotoTerbaru as $foto)
                            <div class="border rounded-lg overflow-hidden">
                                ...
                                <div class="p-2">
                                    ...
                                    <p class="text-xs text-gray-500">{{ $foto->created_at->format('d M Y') }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <div class="p-6">
                @if($masukanTerbaru->count() > 0)
                    <div class="space-y-4">
                        @foreach($masukanTerbaru as $item)
                            <div class="border-l-4 border-blue-500 pl-4 py-2">
                                ...
                                <p class="text-sm text-gray-600 mt-1 line-clamp-2">{{ $item->pesan }}</p>
